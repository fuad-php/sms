...
@section('title', __('app.class_attendance_report'))
...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="bg-white rounded-lg shadow p-6 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-500">{{ __('app.class_teacher') }}</label>
                <p class="mt-1 text-lg font-semibold text-gray-900">
                    {{ $class->teacher ? $class->teacher->user->name : __('app.not_assigned') }}
                </p>
            </div>
        </div>
    </div>
    ...
    @if(request('start_date') && request('end_date'))
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            <div class="bg-white rounded-lg shadow p-6">
                <div class="text-center">
                    ...
                    <div class="text-sm text-gray-500">{{ __('app.total_present') }}</div>
                </div>
            </div>
            <div class="bg-white rounded-lg shadow p-6">
                <div class="text-center">
                    <div class="text-2xl font-bold text-red-600">{{ $summary['total_absent'] ?? 0 }}</div>
                    <div class="text-sm text-gray-500">{{ __('app.total_absent') }}</div>
                </div>
            </div>
            <div class="bg-white rounded-lg shadow p-6">
                <div class="text-center">
                    <div class="text-2xl font-bold text-blue-600">{{ $summary['average_attendance'] ?? 0 }}%</div>
                    <div class="text-sm text-gray-500">{{ __('app.average_attendance') }}</div>
                </div>
            </div>
            <div class="bg-white rounded-lg shadow p-6">
                <div class="text-center">
                    <div class="text-2xl font-bold text-yellow-600">{{ $summary['total_days'] ?? 0 }}</div>
                    <div class="text-sm text-gray-500">{{ __('app.total_days') }}</div>
                </div>
            </div>
        </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('app.present') }}</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('app.absent') }}</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('app.late') }}</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('app.attendance_rate') }}</th>
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <a href="{{ route('attendance.student-report', $student['id']) }}" 
                                   class="text-blue-600 hover:text-blue-900">{{ __('app.view_details') }}</a>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
            ...
            <div class="bg-white rounded-lg shadow p-6">
                <h3 class="text-lg font-medium text-gray-900 mb-4">{{ __('app.top_performers') }}</h3>
                <div class="space-y-3">
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center">
                            ...
                            <div>
                                <div class="text-sm font-medium text-gray-900">{{ $student['user']->name }}</div>
                                <div class="text-xs text-gray-500">{{ $student['attendance_rate'] }}% {{ __('app.attendance') }}</div>
                            </div>
                        </div>
            ...
            <div class="bg-white rounded-lg shadow p-6">
                <h3 class="text-lg font-medium text-gray-900 mb-4">{{ __('app.students_needing_attention') }}</h3>
                <div class="space-y-3">
            ...
            <div class="flex">
                ...
                <div class="ml-3">
                    ...
                    <div class="mt-2 text-sm text-blue-700">
                        <p>{{ __('app.generate_class_attendance_report_instructions') }}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="mt-6">
        <a href="{{ route('attendance.index') }}" 
           class="text-blue-600 hover:text-blue-800 font-medium">
            ← {{ __('app.back_to_attendance_management') }}
        </a>
    </div>
</div>
@endsection
