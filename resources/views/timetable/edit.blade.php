        ...
        <div class="bg-white rounded-lg shadow p-6">
            <form method="POST" action="{{ route('timetable.update', $timetable) }}" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="room" id="room" value="{{ old('room', $timetable->room) }}" placeholder="{{ __('app.enter_room_number') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 @error('room') border-red-500 @enderror">
                    @error('room')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
