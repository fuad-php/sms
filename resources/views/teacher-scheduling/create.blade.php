...
@section('title', __('app.add_schedule'))
...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">{{ __('app.add_schedule') }}</h1>
                <p class="mt-2 text-gray-600">{{ __('app.create_new_teacher_schedule') }}</p>
            </div>
    ...
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-lg shadow">
            ...
            <form method="POST" action="{{ route('teacher-scheduling.store') }}" class="p-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="day_of_week" id="day_of_week" required
                            ...
                            <option value="monday" {{ old('day_of_week') == 'monday' ? 'selected' : '' }}>{{ __('app.monday') }}</option>
                            <option value="tuesday" {{ old('day_of_week') == 'tuesday' ? 'selected' : '' }}>{{ __('app.tuesday') }}</option>
                            <option value="wednesday" {{ old('day_of_week') == 'wednesday' ? 'selected' : '' }}>{{ __('app.wednesday') }}</option>
                            <option value="thursday" {{ old('day_of_week') == 'thursday' ? 'selected' : '' }}>{{ __('app.thursday') }}</option>
                            <option value="friday" {{ old('day_of_week') == 'friday' ? 'selected' : '' }}>{{ __('app.friday') }}</option>
                            <option value="saturday" {{ old('day_of_week') == 'saturday' ? 'selected' : '' }}>{{ __('app.saturday') }}</option>
                            <option value="sunday" {{ old('day_of_week') == 'sunday' ? 'selected' : '' }}>{{ __('app.sunday') }}</option>
                        </select>
                        @error('day_of_week')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select name="schedule_type" id="schedule_type" required
                            ...
                            <option value="regular" {{ old('schedule_type') == 'regular' ? 'selected' : '' }}>{{ __('app.regular') }}</option>
                            <option value="substitute" {{ old('schedule_type') == 'substitute' ? 'selected' : '' }}>{{ __('app.substitute') }}</option>
                            <option value="extra" {{ old('schedule_type') == 'extra' ? 'selected' : '' }}>{{ __('app.extra') }}</option>
                            <option value="remedial" {{ old('schedule_type') == 'remedial' ? 'selected' : '' }}>{{ __('app.remedial') }}</option>
                            <option value="exam_prep" {{ old('schedule_type') == 'exam_prep' ? 'selected' : '' }}>{{ __('app.exam_prep') }}</option>
                        </select>
                        @error('schedule_type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div class="mt-6">
                    ...
                    <textarea name="notes" id="notes" rows="3"
                              ...
                              placeholder="{{ __('app.enter_schedule_notes') }}">{{ old('notes') }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function checkConflicts() {
        ...
        if (teacherId && day && startTime && endTime) {
            fetch(`{{ route('teacher-scheduling.check-conflicts') }}?teacher_id=${teacherId}&day=${day}&start_time=${startTime}&end_time=${endTime}`)
