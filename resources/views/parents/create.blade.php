...
@section('title', __('app.add_parent'))
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 mb-2">{{ __('app.add_parent') }}</h1>
                    <p class="text-gray-600">{{ __('app.create_new_parent_account') }}</p>
                </div>
                <div>
        ...
        <div class="bg-white shadow-sm border border-gray-200 rounded-lg">
            <form method="POST" action="{{ route('parents.store') }}" class="p-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="income_range" id="income_range"
                            ...
                            <option value="low" {{ old('income_range') == 'low' ? 'selected' : '' }}>{{ __('app.low') }}</option>
                            <option value="medium" {{ old('income_range') == 'medium' ? 'selected' : '' }}>{{ __('app.medium') }}</option>
                            <option value="high" {{ old('income_range') == 'high' ? 'selected' : '' }}>{{ __('app.high') }}</option>
                        </select>
                        @error('income_range')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                            ...
                            @foreach($students as $student)
                                <div class="flex items-center space-x-3 p-3 border border-gray-200 rounded-lg">
                                    ...
                                    <select name="relationships[]" class="text-sm border border-gray-300 rounded px-2 py-1">
                                        <option value="father">{{ __('app.father') }}</option>
                                        <option value="mother">{{ __('app.mother') }}</option>
                                        <option value="guardian">{{ __('app.guardian') }}</option>
